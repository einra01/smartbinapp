  ...
  Future<List<MapEntry<String, dynamic>>> _fetchNotifications(String userId) async {
    try {
      ...
      if (acctUpdateSnapshot.snapshot.value != null) {
        final acctUpdates = Map<String, dynamic>.from(acctUpdateSnapshot.snapshot.value as Map);
        allNotifications.addAll(acctUpdates.entries);
      }

      if (assignSnapshot.snapshot.value != null) {
        final assign = Map<String, dynamic>.from(assignSnapshot.snapshot.value as Map);
        allNotifications.addAll(assign.entries);
      }

      if (loggedSnapshot.snapshot.value != null) {
        final loggedUpdates = Map<String, dynamic>.from(loggedSnapshot.snapshot.value as Map);
        allNotifications.addAll(loggedUpdates.entries);
      }

      if (photoSnapshot.snapshot.value != null) {
        final photo = Map<String, dynamic>.from(photoSnapshot.snapshot.value as Map);
        allNotifications.addAll(photo.entries);
      }

      return _filterNotifications(allNotifications); // ✅ Always return a valid list
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: PreferredSize(
        ...
        child: Column(
          children: [
            SizedBox(height: MediaQuery.of(context).size.height * 0.10), // Responsive spacing
            Container(
              color: Colors.amber,
      ...
      body: StreamBuilder(
          ...
          builder: (context, snapshot) {
            ...
            return FutureBuilder<List<MapEntry<String, dynamic>>>(
              future: _fetchNotifications(userId), // ✅ Pass userId properly
                ...
                return SingleChildScrollView(
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        // ✅ New Notifications (Sorted Correctly)
                        Row(
                        ...
                        // ✅ Old Notifications (Sorted Correctly)
