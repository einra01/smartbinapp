...
import 'notification.dart'; // Ensure this is correctly defined
import 'landing.dart'; // Ensure this is correctly defined
import 'account_management.dart'; // Ensure this is correctly defined
    ...
    try {
      ...
      if (pickedFile != null) {
        ...
        if (response.statusCode == 200) {
          ...
          String imageKey = _dbRef.child("images/$userId").push().key!;
          String updateKey = _dbRef.child("notifications/acct_update/$userId").push().key!;
          ...
          // 🔥 Store the NEW image under images/userId/uploadKey
          await _dbRef.child("images/$userId/$imageKey").set({
          ...
          // 🔥 Store notification in notifications/acct_update/userId/updateKey
          DatabaseReference notifRef = FirebaseDatabase.instance.ref("notifications/photo/$userId");

// Generate a new unique key inside "notifications/acct_update/$userId/"
          String notifKey = notifRef.push().key!;

          await notifRef.child(notifKey).set({
            'name': _name,
            'status': "changed profile picture",
            'date': formattedDate,
            'time': formattedTime,
            'userId': userId,
            'msg':'$_name changed profile pictures'
          });



          // 🔥 Update current profile image in users/userId
  ...
  void _logout() async {
    try {
      ...
      print('Current user name: ${_name ?? 'Unknown User'}');

      // Add a logout notification with a unique key
      final notificationsRef = FirebaseDatabase.instance.ref('notifications/logged/$userId');

      // Use push() to create a unique key for each logout notification
            ...
            children: [
              ...
              Column(
                children: [
                  ...
                  // Use MediaQuery to get screen height and adjust spacing
                  SizedBox(height: MediaQuery.of(context).size.height * 0.09), // 10% of the screen height
                  Stack(
                    ...
                    children: [
                      Container(
                        ...
                        child: Column(
                          children: [
                            // Lighter Amber gradient header with reversed colors
                            Container(
                              ...
                              padding: const EdgeInsets.only(bottom: 8), // Push text to bottom
                              child: Align(
