...
class _DevicessState extends State<Devicess> {
  ...
  void _connectToWiFi(String ssid, String password) async {
    try {
      print("🛠️ 1. Starting connection to WiFi: $ssid");
      ...
      print("🔍 2. WiFi connection result: $success");
      ...
      if (success) {
        ...
        await Future.delayed(Duration(seconds: 2)); // Allow WiFi to stabilize
        print("⚡ 3. Fetching device data...");

        await _fetchDeviceData(ssid);

        print("📌 4. Device Data Retrieved: deviceId=$deviceId, deviceName=$deviceName, deviceStatus=$deviceStatus");

        if (deviceId.isEmpty || deviceName.isEmpty) {
          print("❌ 5. Device data is missing! Navigation won't happen.");
          return;
        }

        if (!mounted) {
          print("❌ 6. Context is no longer valid. Skipping navigation.");
          return;
        }

        print("🚀 7. Navigating to DashboardScreens1...");
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => DashboardScreens1(
              deviceId: deviceId,
        ...
        print("❌ 8. WiFi connection failed.");
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("❌ Failed to connect to $ssid")),
        );
      }
    } catch (e) {
      print("💥 Exception in _connectToWiFi: $e");
      ScaffoldMessenger.of(context).showSnackBar(
...
// Add this method to fetch device data
  Future<void> _fetchDeviceData(String ssid) async {
    try {
      // Query to find device by SSID or some other identifier
      // You might need to modify this query based on how you match WiFi SSIDs to devices
      final snapshot = await _devicesRef.orderByChild('ssid').equalTo(ssid).get();

      if (snapshot.exists) {
        Map<dynamic, dynamic> values = snapshot.value as Map<dynamic, dynamic>;
        // Get the first (and hopefully only) matching device
        String key = values.keys.first;
        Map<dynamic, dynamic> deviceData = values[key];
        ...
        print("Found device: $deviceId, $deviceName, $deviceStatus");
      } else {
        print("No device found matching SSID: $ssid");
      }
    } catch (e) {
