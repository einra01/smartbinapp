...
class _DashboardScreenState extends State<DashboardScreens1> with SingleTickerProviderStateMixin {
  ...
  double nonwasteLevel = 0.0;
  double binHeight = 40.0;
  String statusMessage = "Connecting...";
  Timer? _timer;
  String esp32IP = "192.168.1.1"; // Update ESP32 IP address
  @override
  void initState() {
    super.initState();
    fetchWasteLevel();
  ...
  //nonbio waste esp32 connection
  Future<void> fetchnonWasteLevel() async {
    String url = "http://$esp32IP/nonwaste_level"; // ESP32 endpoint
    ...
    try {
      ...
      if (response.statusCode == 200) {
        var data = jsonDecode(response.body);
        setState(() {
          nonwasteLevel = data["nonwasteLevel"].toDouble();
          statusMessage = "Connected ✅";
        });
      } else {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    double percentages = (nonwasteLevel / binHeight) * 100;
